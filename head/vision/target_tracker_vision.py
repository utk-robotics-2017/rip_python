...
class TargetTrackerVision:
    constants = Constants()
    kMinTargetWidth = constants.constants.target_tracker.vision.min_target_width
    kMaxTargetWidth = constants.constants.target_tracker.vision.max_target_width
    kMinTargetHeight = constants.constants.target_tracker.vision.min_target_height
    kMaxTargetHeight = constants.constants.target_tracker.vision.max_target_height
    kNearlyHorizontalSlope = 1 / 1.25
    kNearlyVerticalSlope = 1.25
    kMinFullness = constants.constants.target_tracker.vision.min_fullness
    kMaxFullness = constants.constants.target_tracker.vision.max_fullness
    ...
    def __init__(self, camera):
        lower_h = self.constants.constants.target_tracker.lower_hue
        lower_s = self.constants.constants.target_tracker.lower_saturation
        lower_v = self.constants.constants.target_tracker.lower_value
        self.lower_range = np.array([lower_h, lower_s, lower_v])

        upper_h = self.constants.constants.target_tracker.upper_hue
        upper_s = self.constants.constants.target_tracker.upper_saturation
        upper_v = self.constants.constants.target_tracker.upper_value
    ...
    def process_image(self, image=None, display=False, save=False):
        ...
        for contour in contours:
            ...
            if len(poly) == 4 and cv2.isContourConvex(poly):
                target = TargetTrackerVision.Target()
                min_x = float("inf")
                max_x = -float("inf")
                min_y = float("inf")
                max_y = -float("inf")
